...
export const curseforgeHash = (data: Uint8Array): number => {
  ...
  while (len >= 4) {
    ...
    tmp = (tmp & 0xffff) * 0x5bd1e995 + ((((tmp >>> 16) * 0x5bd1e995) & 0xffff) << 16);
    tmp ^= tmp >>> 24;
    tmp = (tmp & 0xffff) * 0x5bd1e995 + ((((tmp >>> 16) * 0x5bd1e995) & 0xffff) << 16);

    hash = ((hash & 0xffff) * 0x5bd1e995 + ((((hash >>> 16) * 0x5bd1e995) & 0xffff) << 16)) ^ tmp;
...
export const modrinthHash = async (f: Uint8Array): Promise<string> => {
  const ha = await window.crypto.subtle.digest("SHA-512", f as BufferSource);
